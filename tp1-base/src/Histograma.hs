...
agregar x (Histograma l u ls)
                            ...
                            | otherwise = Histograma l u (actualizarElem posicion (+1) ls)
          where
            casilleros = length ls - 2 -- revisar xq no poner el ultimo
            long_intervalo = (u - l) / fromIntegral casilleros
            posicion = floor ((x - l) / long_intervalo) + 1


-- | Arma un histograma a partir de una lista de números reales con la cantidad de casilleros y rango indicados.
histograma :: Int -> (Float, Float) -> [Float] -> Histograma
histograma n r xs = foldl (\acumulador valor -> agregar valor acumulador) (vacio n r) xs
...
casilleros (Histograma inicio ancho valores) = zipWith4 Casillero rangos_inferiores rangos_maximos valores porcentajes
            where
              total_casilleros = length valores - 2 -- Sera necesario restar el ultimo?
              sumatoria = fromIntegral (sum valores)
              calculo_porcentaje c = if sumatoria == 0 then 0 else (fromIntegral c / sumatoria) * 100.0
              porcentajes = map calculo_porcentaje valores
              rangos_inferiores = [inicio + fromIntegral i * ancho | i <- [0 .. total_casilleros-1]]
              rangos_maximos = [inicio + fromIntegral (i+1) * ancho | i <- [0 .. total_casilleros-1]]
